...
namespace Auto.Data {
    public class AutoCsvFileDatabase : IAutoDatabase
    {
        ...
        private void ResolveReferences()
        {
            ...
            {
                veh.Owners = owners.Values.Where(v => v.RegistrationCode == veh.Registration).ToList();
                foreach (var owner in veh.Owners) owner.VehicleCode = veh;
            }
        }
        ...
        private void ReadOwnersFromCsvFile(string filename)
        {
            ...
            foreach (var line in File.ReadAllLines(filePath))
            {
                ...
                {
                    Name = tokens[0],
                    NumberPhone = tokens[1],
                    Address = tokens[2],
                    RegistrationCode = tokens[3]
                };
        ...
        {
            owner.RegistrationCode = owner.VehicleCode.Registration;
        ...
        {
            var vehicle = FindVehicle(owner.RegistrationCode);
            vehicle.Owners.Remove(owner);
            owners.Remove(owner.Name);
        }
